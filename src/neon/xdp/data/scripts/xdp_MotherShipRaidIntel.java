...
public class xdp_MotherShipRaidIntel extends BaseIntelPlugin implements ColonyPlayerHostileActListener {
    ...
    public xdp_MotherShipRaidIntel(MarketAPI bombmarket, ArrayList<Integer> reward) {
        ...
        this.sprite = Global.getSettings().getSpriteName("factions", Factions.DERELICT);
    }

    @Override
    public void createIntelInfo(TooltipMakerAPI info, IntelInfoPlugin.ListInfoMode mode) {
        if (memory != null && memory.getInt(key) == 1) {
            info.addTitle(title);
            info.addPara("Return to any Mothership to receive the rewards.", 5f);
        } else {
            info.addTitle(title);
            if (bombmarket != null) {
                String systemName = bombmarket.getStarSystem() != null ? bombmarket.getStarSystem().getName() : "unknown system";
                info.addPara("Bombard " + bombmarket.getName() + " in the " + systemName + ".", 5f);
            }
        }
    }
    ...
    public void reportTacticalBombardmentFinished(InteractionDialogAPI dialog,
                                                  MarketAPI market,
                                                  MarketCMD.TempData actionData) {
        if (market != null && market == bombmarket && memory != null) {
            memory.set(key, 1, 0f);
    ...
    public boolean isDone() {
        return (memory != null && memory.getInt(key) == 2);
    }
