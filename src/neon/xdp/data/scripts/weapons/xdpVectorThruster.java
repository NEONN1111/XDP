...
public class xdpVectorThruster implements EveryFrameWeaponEffectPlugin {
    ...
    private Vector2f size= new Vector2f(30,70);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if(!runOnce){
            ...
            EMGINES=HOST.getEngineController();
            ...
            TURN_RIGHT_ANGLE=MathUtils.clampAngle(VectorUtils.getAngle(HOST.getLocation(), weapon.getLocation()));
            ...
            THRUST_TO_TURN=smooth(MathUtils.getDistance(HOST.getLocation(), weapon.getLocation())/HOST.getCollisionRadius());
        }
        ...
        if(time>=FREQ){
            ...
            if(thrust>0){
                ...
                if(!turn){
                    //thrust only, easy.
                    thrust(weapon, accelerateAngle, thrust*(HOST.getMutableStats().getAcceleration().computeMultMod()), SMOOTH_THRUSTING);
                } else {
                    if(!accel){                        
                        //turn only, easy too.
                        thrust(weapon, turnAngle, thrust*(HOST.getMutableStats().getTurnAcceleration().computeMultMod()), SMOOTH_THRUSTING);
                        ...
                        float clampedThrustToTurn=THRUST_TO_TURN*Math.min(1, Math.abs(HOST.getAngularVelocity())/10);
                        ...
                        combinedThrust*=(HOST.getMutableStats().getTurnAcceleration().computeMultMod() + HOST.getMutableStats().getAcceleration().computeMultMod())/2;
    ...
    private void rotate(WeaponAPI weapon, float angle, float thrust, float smooth){
        //target angle
        float aim=angle+HOST.getFacing();
    ...
    private void thrust(WeaponAPI weapon, float angle, float thrust, float smooth){
        ...
        //target angle
        float aim=angle+HOST.getFacing();//+SHIP.getFacing();
        float length=thrust;        
