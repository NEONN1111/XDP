...
public class xdp_BlinkDriveAI implements ShipSystemAIScript {
    ...
    public void advance(float amount, Vector2f missileDangerDir, Vector2f collisionDangerDir, ShipAPI target) {
        ...
        if (tracker.intervalElapsed()) {
            ...
            List<ShipAPI> possibleTargets = XDP_Util.getShipsWithinRange(ship.getLocation(), 5000f);
        ...
        if (tracker2.intervalElapsed()) {
            if (!AIUtils.canUseSystemThisFrame(ship) || ship.getShield().isOff()) {
                return;
            }

            if (tracker2.intervalElapsed()) {
                if (!AIUtils.canUseSystemThisFrame(ship) || ship.getShield().isOn()) {
                    return;
                }
                ...
                switch (ship.getHullSpec().getBaseHullId()) {
                    case "swp_arcade_superzero" -> maxDistance += 150f;
                    case "swp_arcade_hyperzero" -> maxDistance += 300f;
                    default -> {
                    }
                }
