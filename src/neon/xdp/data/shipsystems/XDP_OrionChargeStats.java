...
public class XDP_OrionChargeStats extends BaseShipSystemScript {
	...
	public static class OrionDeviceParams {
		...
		public float impactAccel = 9000f;
		public float impactRateMult = 1f;
	...
	protected EveryFrameCombatPlugin createBombImpactPlugin(final ShipAPI ship, final WeaponSlotAPI launchSlot,
		...
		return new BaseEveryFrameCombatPlugin() {
			...
			public void advance(float amount, List<InputEventAPI> events) {
				...
				String impactCounterId = "oc_system_counter";
				...
				if (bomb.isFizzling()) {
					if (!triggered) {
						...
						triggered = true;
						ship.getMutableStats().getDynamic().getMod(impactCounterId).modifyFlat("oc_launch_" + launchLoc, 1f);
...
//					}
				...
				if ((!triggered && elapsed > 1f) || done) {
					...
					ship.getMutableStats().getDynamic().getMod(impactCounterId).unmodifyFlat("oc_launch_" + launchLoc);
				}
			}
		};
	}
